...
export default function Catalog() {
  ...
  const [selectedServiceId, setSelectedServiceId] = useState(demoCatalogServices[0]?.id ?? '')

  const filteredServices = useMemo(
    () =>
      demoCatalogServices.filter(
        (service) =>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-3">
              {['AI release-risk forecast', 'Evidence pack export', 'Ownership drift detection'].map((item) => (
                <span
                  key={item}
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <StatusBadge status={service.releaseState === 'blocked' ? 'degraded' : 'completed'} size="sm" />
                      </div>
        ...
        >
          {selectedService ? (
            <div className="space-y-5">
              <div className="flex flex-wrap items-center gap-2">
                <StatusBadge
                  status={selectedService.releaseState === 'blocked' ? 'degraded' : 'completed'}
                  label={selectedService.releaseState}
                />
                <StatusBadge status={selectedService.riskLevel === 'high' ? 'unhealthy' : selectedService.riskLevel === 'medium' ? 'degraded' : 'healthy'} label={`${selectedService.riskLevel} risk`} />
              ...
              <div className="space-y-3 rounded-2xl border border-gray-200 p-4 dark:border-dark-700">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600 dark:text-gray-400">Release readiness</span>
                  <span className="font-semibold text-gray-900 dark:text-white">{selectedService.healthScore}%</span>
                </div>
                <ProgressBar
                  value={selectedService.healthScore}
                  color={selectedService.releaseState === 'blocked' ? 'danger' : selectedService.releaseState === 'watch' ? 'warning' : 'success'}
                />
                <div className="grid gap-2 text-sm text-gray-600 dark:text-gray-400">
                  ...
                  <p>
                    <span className="font-medium text-gray-900 dark:text-white">Evidence:</span>{' '}
                    {selectedService.evidence.join(', ')}
                  </p>
                </div>
              </div>
              ...
              <div className="grid gap-3">
                <div className="rounded-2xl bg-gray-50 p-4 dark:bg-dark-700/40">
                  <p className="text-xs uppercase tracking-[0.2em] text-gray-500 dark:text-gray-400">AI benefits</p>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {selectedService.aiBenefits.map((benefit) => (
                      <span
                        key={benefit}
                ...
                <div className="rounded-2xl bg-gray-50 p-4 dark:bg-dark-700/40">
                  <p className="text-xs uppercase tracking-[0.2em] text-gray-500 dark:text-gray-400">Control summary</p>
                  <p className="mt-2 text-sm leading-6 text-gray-600 dark:text-gray-300">
                    The catalog keeps the service release-ready by pairing ownership, policy checks,
                    and evidence capture. This is where the product closes the gap between service
                    discovery and compliance workflows.
                  </p>
                </div>
              </div>
            </div>
